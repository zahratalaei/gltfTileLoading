...
function handleMouseMove(movement) {
    if (selectedPoints.length > 0) {
        const middlePoint = getPosition(movement.endPosition);
        if (middlePoint) {
            if (tempPolyline) {
            ...
            tempPolyline = viewer.entities.add({
                polyline: {
                    positions: [selectedPoints[selectedPoints.length - 1].position, middlePoint],
...
function handleClick(click) {
    ...
    if (selectedPointIDs.includes(uniqueID)) {
        ...
        console.log('Point added:', clickedPoint);
