import { createDataURIfromBuffers } from "./createBuffer.mjs";
...
const dataURL = createDataURIfromBuffers(data);

export function getCombinedBufferFromDataURL(dataURL) {
...
// Usage example:
// const decodedFeatureIds = decodeFeatureIdsBufferBase64String(featureIdsBufferBase64String);
// console.log("Decoded Feature IDs:", decodedFeatureIds);
...
// const decodedAttributes = decodeAttributes(bufferString, byteLengthsArray);
...
export function extractBuffersFromCombined(dataURL) {
  const combinedBuffer = getCombinedBufferFromDataURL(dataURL);
  ...
  return {
    lengthsBuffer, // This is now directly the sliced buffer
    cartesianBuffer,
    featureIdsBuffer,
    bufferString,
    byteLengthsBuffer,
...
//function to give all decoded data from dataURL
function decodeData(dataURL) {
  const extractedBuffers = extractBuffersFromCombined(dataURL);
  ...
  const decodedLengthBuffer = decodeFeatureIdsBufferBase64String(
    extractedBuffers.lengthsBuffer
  );
...
const decodedData = decodeData(dataURL);
