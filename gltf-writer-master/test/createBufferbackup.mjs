...
export function createDataURIfromData(data) {
  ...
  const localizedData = data.map(item => ({
    ...
    cartesian: item.cartesian.map(point =>
      localizeCoordinates(point, tileCenter)
    ) // Localize each point
  }));
  ...
  const flatCartesiansArray = new Float32Array(
  ...
  return {
    ...
    byteLengthsArray: byteLengthsArray,
...
// Example usage:
const result = createDataURIfromData(data);
